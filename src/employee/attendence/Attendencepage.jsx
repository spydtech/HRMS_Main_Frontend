...
const AttendancePage = () => {
  ...
  const [currentDateTime, setCurrentDateTime] = useState(new Date().toLocaleTimeString());
  ...
  const [employeeId, setEmployeeId] = useState(auth.employee?.employeeId || ""); // Safeguard against undefined
  const [employeeName, setEmployeeName] = useState(`${auth.employee?.firstName.toUpperCase() || ""} ${auth.employee?.lastName.toUpperCase() || ""}`); // Safeguard against undefined
    ...
    if (isPunchedIn) {
      timer = setInterval(() => {
        setElapsedTime((prevElapsedTime) => {
          const totalSeconds = prevElapsedTime.hours * 3600 + prevElapsedTime.minutes * 60 + prevElapsedTime.seconds + 1;
  ...
  const handlePunchButtonClick = async () => {
    if (isPunchedIn) {
      ...
      const totalProductionHours = production.hours + production.minutes / 60 + production.seconds / 3600;
      const totalBreakHours = breakDuration.hours + breakDuration.minutes / 60 + breakDuration.seconds / 3600;

      const overtime = totalProductionHours > officeHours ? totalProductionHours - officeHours : 0;
  ...
  //   if (!isPunchedIn) {
  //     // Reset elapsed time when punching in
  //     setElapsedTime({ hours: 0, minutes: 0, seconds: 0 });
  //   }
  // };
  ...
  //   const dateMatch = entry.punchIn.toDateString() === searchDate.toDateString();
  //   const nameMatch = entry.employeeName.toLowerCase().includes(searchQuery.toLowerCase());
  //   return dateMatch && nameMatch;
  // }); 

  const filteredData = attendanceData.filter(entry => {
  // Ensure entry.employeeName is defined and handle the case where it might be undefined
  const nameMatch = entry.employeeName ? entry.employeeName.toLowerCase().includes(searchQuery.toLowerCase()) : false;
  const dateMatch = entry.punchIn.toDateString() === searchDate.toDateString();
  return dateMatch && nameMatch;
});
 ...
 {/* <div className="bg-white w-[500px] shadow-md rounded-lg p-6 mb-6 flex flex-col space-y-4">
     ...
     <div className="bg-white w-[500px] shadow-md rounded-lg p-6 mb-6 flex flex-col space-y-4">
      ...
      {/* <div className="bg-[#0098F1] text-white p-4 rounded-lg mb-4">
       ...
       <div className="bg-[#0098F1] text-white p-4 rounded-lg mb-4">
        <h3 className="text-[20px] font-semibold">Punch In at</h3>
        <p className="text-[20px] font-normal">{currentDateTime.toLocaleString()}</p>      </div>
      ...
      <div className="relative w-[200px] h-[200px] ml-[60px]">
        ...
        <p className="font-semibold text-[15px] mt-[110px] ml-[60px] text-[#E65F2B]">
        ...
        <div className="flex space-x-4">
          ...
          <label className="flex-1">
            ...
            <select
              ...
              className="p-2  bg-white  border border-white-200  text-[#E65F2B] outline-transparent  text-[20px] rounded-3xl w-[200px] h-[60px]"
            >
              <option value="text-">Select month</option>
          ...
          <label className="flex-1">
            <input
              ...
              className="p-2  bg-white  border border-white-200  text-[#E65F2B] outline-transparent  text-[20px] rounded-3xl w-[180px] h-[60px]"
            />
          </label>
          <label className="flex-1">
            <DatePicker
          ...
          <input
              ...
              className="p-2 border border-gray-200 mt-[10px] w-[200px] outline-none text-[#FFFFFF] font-normal text-[15px] bg-[#E65F2B]  placeholder:text-[#FFFFFF] rounded-lg "
            />
        </div>
      </div>
      <label className="flex-1">
  ...
  <table className="min-w-full divide-y divide-red-200">
    ...
    <tbody className="bg-white divide-y divide-gray-200">
      {filteredData.map((entry, index) => (
        <tr key={index}>
          <td className="px-4 py-2 whitespace-nowrap  font-medium  border border-[#E65F2B]">{entry.employeeId}</td>
          <td className="px-4 py-2 whitespace-nowrap text-[16px]  font-semibold text-[#E65F2B] border border-[#E65F2B]">{entry.employeeName}</td>
          ...
          <td className="px-4 py-2 whitespace-nowrap text-[16px] font-semibold  text-[#E65F2B] border border-[#E65F2B]">{`${entry.production.hours}h ${entry.production.minutes}m ${entry.production.seconds}s`}</td>
          <td className="px-4 py-2 whitespace-nowrap text-[16px]  font-semibold border border-[#E65F2B] text-[#E65F2B]">{`${entry.breakDuration.hours}h ${entry.breakDuration.minutes}m ${entry.breakDuration.seconds}s`}</td>
